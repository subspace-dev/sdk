...
local function leave_server(msg)
    ...
    if utils.profiles.is_bot(userId) then
        local bot = utils.bots.get(userId)
        assert(bot, "404|bot not found")
        if bot.servers[serverId] then
            isMember = bot.servers[serverId].approved
        else
            isMember = false
        end
    else
        ...
        if profile.servers[serverId] then
            isMember = profile.servers[serverId].approved
        else
            isMember = false
        end
    end
    assert(isMember, "404|user is not a member of the server")

    -- redirect leave request to server
    send({
        target = serverId,
...
local function approve_remove_member(msg)
    local serverId = msg.from
    local userId = utils.var_or_nil(msg["user-id"])
    local isBot = utils.profiles.is_bot(userId)
    ...
    msg.reply({
        action = "approve-remove-member-response",
        status = helpers.status.success,
    })
end

Handlers.add("approve-remove-member", function(msg)
    utils.handle_run(approve_remove_member, msg)
end)
