...
local function leave_server(msg)
    ...
    -- For now, we'll let subspace.lua handle this validation
    -- since it has access to server ownership information
    ...
    -- Use the standard subspace leave-server flow instead of direct remove-member
    -- subspace.lua will handle the owner validation
    send({
        target = subspace_id,
        action = "leave-server",
        ["server-id"] = serverId
